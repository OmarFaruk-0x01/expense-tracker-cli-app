...
class AddIncome extends Feature
{
    ...
    function run(): void
    {
        ...
        $categories = $this->state->getIncomeCategories();
        $this->view->renderList($categories, fn ($category, $index) => sprintf("%s. $category\n", $index+1));

        $category = (int) $this->repeatAsking(placeholder: "Enter Category Number: ",
            closure: fn ($input) => (int) $input <= 0 || (int) $input > count($categories)) - 1;

        try {
            $this->state->addIncome(new Income($money, $categories[$category]));
